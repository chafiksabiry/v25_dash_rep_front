...
const globalState = {
  ...
  setVisible: (visible: boolean) => {
    console.log("👁️ Setting visibility:", visible);
  ...
  setCallEnded: (ended: boolean) => {
    console.log("🔚 Setting call ended:", ended);
    globalState.callEnded = ended;
    globalState.notifySubscribers();
  },
  setCallDetails: (callSid: string, agentId: string) => {
    console.log("🎯 Setting call details - CallSid:", callSid, "AgentId:", agentId);
  ...
  saveCallToDB: async () => {
    ...
    try {
      ...
      const result = await axios.post(`${import.meta.env.VITE_API_URL_CALL}/api/calls/call-details`, {
        ...
        userId: globalState.currentUser?.id || "6807abfc2c1ca099fe2b13c5"
      });
      const call = result.data.data;
      console.log("📞 Call details retrieved:", call);
      ...
      console.log("💾 Storing call in DB...");
...
console.log("🚀 Initializing AIAssistant API...");
if (typeof window !== 'undefined') {
  ...
  console.log("✅ AIAssistant API initialized successfully");
} else {
  console.warn("⚠️ Window object not available, AIAssistant API not initialized");
}
