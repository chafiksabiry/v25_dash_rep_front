  ...
  return (
    <div className="space-y-10 pb-10 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
         {/* Professional Focus */}
         <div className="bg-white/60 backdrop-blur-md rounded-[40px] border border-white/80 shadow-xl shadow-slate-200/30 overflow-hidden">
            <div className="px-10 py-8 border-b border-slate-100 flex items-center justify-between bg-white/40">
              <h2 className="text-lg font-black text-slate-900 uppercase tracking-widest flex items-center gap-2">
                <Users className="w-5 h-5 text-harx-500" />
                Professional Summary
              </h2>
            </div>
            <div className="p-10">
              <p className="text-slate-700 leading-relaxed font-medium italic text-lg">
                "{profile?.professionalSummary?.profileDescription || 'Showcase your expertise by updating your profile summary.'}"
              </p>
            </div>
         </div>

         {/* Skills */}
         <div className="bg-white/60 backdrop-blur-md rounded-[40px] border border-white/80 shadow-xl shadow-slate-200/30 p-10">
            <h2 className="text-xs font-black text-slate-400 uppercase tracking-[0.3em] mb-8">Verified Skills</h2>
            <div className="flex flex-wrap gap-3">
              {[...(profile?.skills?.technical || []), ...(profile?.skills?.professional || [])].slice(0, 15).map((skill, idx) => (
                <span key={idx} className="px-4 py-2 bg-slate-50 text-slate-600 rounded-xl text-[10px] font-black uppercase tracking-wider border border-slate-100">
                  {typeof skill === 'string' ? skill : (skill.name || skill.skill?.name || 'Skill')}
                </span>
              ))}
            </div>
         </div>
